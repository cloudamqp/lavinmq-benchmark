...
module "load_generator" {
  ...
  # Install lavinmq (for lavinmqperf, not used here but keeps instance consistent)
  install_crystal = true
  lavinmq_version = ""
  install_lavinmq = true
  stop_lavinmq    = true
...
# Run multiple MQTT latency tests
resource "terraform_data" "mqtt_latency_tests" {
  connection {
    type  = "ssh"
    user  = "ubuntu"
  ...
  provisioner "file" {
    source      = "../../../scripts/run_mqtt_latency_tests.sh"
    destination = "/home/ubuntu/run_mqtt_latency_tests.sh"
  }

  # Execute the multiple test runs
  provisioner "remote-exec" {
    inline = [
      "chmod +x /home/ubuntu/mqtt_bench.sh /home/ubuntu/run_mqtt_latency_tests.sh",
      format("/home/ubuntu/run_mqtt_latency_tests.sh %s %s %s %s %s",
...
output "results_file_path" {
  ...
  value       = "/home/ubuntu/mqtt_latency_results.md"
}

output "ssh_view_results_command" {
  description = "Command to SSH and view the results"
  value       = format("ssh -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/tmp/known-hosts ubuntu@%s 'cat /home/ubuntu/mqtt_latency_results.md'", module.load_generator.public_dns)
}

output "scp_download_results_command" {
  description = "Command to download the results file using SCP"
  value       = format("scp -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=/tmp/known-hosts ubuntu@%s:/home/ubuntu/mqtt_latency_results.md ./mqtt_latency_results.md", module.load_generator.public_dns)
}
