...
resource "terraform_data" "multiple_throughput_tests" {
  connection {
    type  = "ssh"
    user  = "ubuntu"
  ...
  provisioner "file" {
    source      = "../../../scripts/run_multiple_throughput_tests.sh"
    destination = "/home/ubuntu/run_multiple_throughput_tests.sh"
  }
