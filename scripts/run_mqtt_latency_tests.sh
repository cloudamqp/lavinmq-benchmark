...
# Script to run multiple MQTT latency tests with different message sizes and rate limits
# Usage: ./run_mqtt_latency_tests.sh <broker_ip> [size1,size2,...] [rate1,rate2,...] [duration] [broker_instance_type]
...
OUTPUT_FILE="/home/ubuntu/mqtt_latency_results.md"
TEMP_DIR="/tmp/mqtt_latency_test_$$"
...
for SIZE in "${SIZE_ARRAY[@]}"; do
  ...
  for RATE in "${RATE_ARRAY[@]}"; do
    ...
    # Run the latency test via mqtt_bench.sh wrapper
    echo "Running MQTT latency test (size=$SIZE, rate=$RATE, duration=$DURATION seconds)..."
    TEST_OUTPUT=$("$SCRIPT_DIR/mqtt_bench.sh" throughput -z "$DURATION" -x 1 -y 1 -s "$SIZE" -r "$RATE" --measure-latency \
